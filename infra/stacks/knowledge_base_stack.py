...
class KnowledgeBaseStack(Stack):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Only set explicit names for globally-unique resources when suffix provided
        global_name_prefix = f"{name_prefix}-{resource_suffix}" if resource_suffix else name_prefix

        vector_bucket_name = f"{global_name_prefix}-kb-vectors"
        index_name = f"{global_name_prefix}-sops-index"

        # S3 bucket for SOP documents (regular bucket - no explicit name needed)
        ...
        self.data_source = bedrock.CfnDataSource(
            self, "DataSource",
            name=f"{name_prefix}-sops-source",
        ...
        CfnOutput(self, "KnowledgeBaseId",
            ...
            export_name=f"{name_prefix}-kb-id",
        )
