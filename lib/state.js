...
const factoryState = {
  ...
  analysisConfig: {
    ...
    summaryIntervalMs: parseInt(process.env.AGENT_SUMMARY_INTERVAL_MS, 10) || 1800000,  // Tier 3: 30 minutes
    changeThresholdPct: parseFloat(process.env.AGENT_CHANGE_THRESHOLD_PCT) || 5,         // % change to trigger Tier 2
    anomalyCacheTtlMs: parseInt(process.env.ANOMALY_CACHE_TTL_MS, 10) || 60 * 60 * 1000, // 60 minutes (P2)
    minAnomalySeverity: 'medium', // Skip 'low' severity anomalies (P1)
    isPaused: false, // Pause state for agentic loop (suspend without destroying state)
    dailyTokenBudget: parseInt(process.env.AGENT_DAILY_TOKEN_BUDGET, 10) || 500000  // Daily input token limit
  },
