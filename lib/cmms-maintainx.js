...
class MaintainXProvider extends CMmsProvider {
   ...
   * @returns {string} MaintainX priority: 'LOW' | 'MEDIUM' | 'HIGH' | 'URGENT'
   * @private
   */
  ...
  async createWorkOrder(anomaly, equipment) {
    ...
    try {
      ...
      // NOTE: Field names are placeholders - adjust based on actual MaintainX API docs
      const payload = {
        title: title.substring(0, 200), // Truncate if too long
        description,
        priority,
        status: 'OPEN', // Initial status
        // Optional fields - configure these in environment or config
        ...(this.defaultLocationId && { locationId: this.defaultLocationId }),
        ...(this.defaultAssigneeId && { assigneeId: this.defaultAssigneeId }),
        // Custom fields for filtering/tracking
        customFields: {
          source: 'EdgeMind-AI',
  ...
  async listRecentWorkOrders(limit = 10) {
    ...
    try {
      // Query params - adjust based on actual MaintainX API
      const query = new URLSearchParams({
        limit: limit.toString(),
        sort: 'createdAt:desc',
        // Filter for AI-created work orders if supported
        // source: 'EdgeMind-AI'
      });

      const response = await this._makeRequest('GET', `/workorders?${query.toString()}`);
