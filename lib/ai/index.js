...
/** Max output tokens for single-shot Tier 2 targeted analysis Comment*/
...
function summarizeTrends(trends) {
  ...
  Object.entries(grouped).slice(0, 30).forEach(([key, values]) => {
    // Issue #5 fix: Handle single-point measurements
    if (values.length === 1) {
      lines.push(`${key}: current=${values[0].value.toFixed(2)} (single reading)`);
    } else if (values.length >= 2) {
      const first = values[0].value;
      const last = values[values.length - 1].value;
      // Issue #6 fix: Handle division by zero with informative message
      const change = first !== 0
        ? ((last - first) / first * 100).toFixed(1)
        : (last !== 0 ? `new(${last.toFixed(1)})` : '0.0');
...
/**
 * Fetch similar historical anomalies from vector store and format as prompt context.
 * Returns empty string if vector store unavailable or no history found.
 */
async function getHistoricalContext(queryText) {
  if (!vectorStoreInstance || !vectorStoreInstance.isReady()) {
    return '';
  }

  try {
    const similar = await vectorStoreInstance.findSimilarAnomalies(queryText, 5);
    if (!similar || similar.length === 0) {
      return '';
    }

    const formatted = similar.map((item, i) => {
      const m = item.metadata;
      const age = m.timestamp ? getRelativeAge(m.timestamp) : 'unknown';
      return `${i + 1}. [${m.severity?.toUpperCase()}] ${m.enterprise} - ${m.description} (${age} ago, metric: ${m.metric}, value: ${m.actual_value}, threshold: ${m.threshold})`;
    }).join('\n');

    return `\n## Historical Anomaly Context (Similar Past Events)\n\nThe following similar anomalies were detected previously. Use these to identify recurring patterns or worsening trends:\n\n${formatted}\n`;
  } catch (error) {
    console.warn('Failed to fetch historical anomaly context:', error.message);
    return '';
  }
}

/**
 * Get human-readable relative age from ISO timestamp.
 */
function getRelativeAge(isoTimestamp) {
 ...
 * @param {string} historicalContext - Historical anomaly context from vector store (optional)
 * @returns {string} Complete prompt
 */
function buildTargetedPromptWithData(changes, trendSummary, toolData, historicalContext = '') {
 ...
 * @returns {string} Complete prompt
 */
function buildSummaryPromptWithData(enterpriseFocus, trendSummary, domainContext, toolData, historicalContext = '') {
...
async function runTargetedAnalysis(trends, changes) {
  ...
  try {
    ...
    // RAG: Fetch similar historical anomalies for context
    const changeQuery = changes.map(c =>
      c.type === 'metric_change'
        ? `${c.enterprise} ${c.measurement} ${c.direction} change`
        : `${c.equipment} state change ${c.previousState} to ${c.currentState}`
    ).join('. ');
    const historicalContext = await getHistoricalContext(changeQuery);

    const prompt = buildTargetedPromptWithData(changes, trendSummary, toolData, historicalContext);
...
async function runScheduledSummary() {
  ...
  try {
    ...
    // RAG: Fetch similar historical anomalies for context
    const summaryQuery = `${enterpriseFocus} factory performance OEE anomalies trends`;
    const historicalContext = await getHistoricalContext(summaryQuery);

    const prompt = buildSummaryPromptWithData(enterpriseFocus, trendSummary, domainContext, toolData, historicalContext);
  ...
  if (insight.anomalies && Array.isArray(insight.anomalies)) {
    ...
    insight.anomalies.forEach(anomaly => {
      ...
      if (isDuplicateAnomaly(enterprise, metric, anomalyType)) {
        console.log(`🔁 Skipping duplicate anomaly: ${enterprise}/${metric}/${anomalyType}`);
        return;
      }
    ...
    while (factoryState.anomalies.length > 50) {
...
function stopAgenticLoop() {
  ...
  if (demoAnalysisTimeout) {
    ...
    console.log('🛑 Stopped pending demo analysis timeout');
  }
...

{
  ...
  "confidence": 0.0-1.0
    ...
    try {
      ...
      if (timeoutError.message.includes('timeout')) {
        console.warn(`⚠️ Demo analysis Bedrock timeout (${BEDROCK_TIMEOUT_MS}ms)`);
      }
      throw timeoutError;
    }
