...
/**
 * Bedrock API timeout per call (12 seconds)
 * Prevents individual API calls from consuming the entire 30s budget
 */
...
1. If you see concerning OEE metrics (e.g., "Enterprise B availability is 72%"), ALWAYS use tools to investigate WHY
...
4. If Quality is low or defects are high, use get_downtime_analysis to quantify the problem
...

{
  ...
  try {
    ...
    let toolCallCount = 0;
    const MAX_TOOL_CALLS = 3; // Limit to 3 tool calls to stay within 30s budget
