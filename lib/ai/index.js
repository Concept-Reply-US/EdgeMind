...
function buildBedrockPayload(messages, options = {}) {
  ...
  if (isNovaModel(modelId)) {
    ...
    // Nova doesn't support tools the same way - omit for now
    return payload;
  }
...

{
  ...
  try {
    ...
    while (true) {
      ...
      // Check if Claude wants to use tools (Nova doesn't support tools)
      const toolUseBlocks = content.filter(block => block.type === 'tool_use');

      if (toolUseBlocks.length === 0) {
        // No tools requested - model provided final analysis
