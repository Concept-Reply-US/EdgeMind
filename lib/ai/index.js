...
function buildBedrockPayload(messages, options = {}) {
  const { maxTokens = 2000, tools, system } = options;
...
/**
 * Get stop reason from response (handles Claude and Nova formats)
 */
function getStopReason(responseBody) {
  // Claude: stop_reason, Nova: stopReason
  return responseBody.stop_reason || responseBody.stopReason || 'unknown';
}
