...
/**
 * Extract tool use blocks from response content (handles Claude and Nova formats)
 * Claude: { type: 'tool_use', id, name, input }
 * Nova: { toolUse: { toolUseId, name, input } }
 */
function extractToolUseBlocks(content) {
  return content.filter(block => block.type === 'tool_use' || block.toolUse);
}

/**
 * Get tool details from a tool use block (handles Claude and Nova formats)
 */
function getToolDetails(toolUseBlock) {
  const toolUse = toolUseBlock.toolUse || toolUseBlock;
  return {
...
/**
 * Format a tool result for the model (handles Claude and Nova formats)
 */
function formatToolResult(toolId, result) {
  if (isNovaModel(CONFIG.bedrock.modelId)) {
    return { toolResult: { toolUseId: toolId, content: [{ text: JSON.stringify(result) }] } };
  }
  return { type: 'tool_result', tool_use_id: toolId, content: JSON.stringify(result) };
}

/**
 * Format a text message block for the model
 */
...

{
  ...
  try {
    ...
    // Guard: ensure Bedrock client is initialized
    ...
    while (true) {
      const payload = buildBedrockPayload(messages, { maxTokens: 2000, tools: TOOL_DEFINITIONS });
      ...
      console.log('[AI] Raw Bedrock response:', JSON.stringify(responseBody).slice(0, 1000));
      const content = extractResponseContent(responseBody);

      if (!content || content.length === 0) {
        console.error('[AI] Unexpected Bedrock response format - no content array');
        return buildInsightFromAnalysis(
          { summary: 'Analysis temporarily unavailable - unexpected API response', confidence: 0.1 },
          { dataPoints: trends.length, apiError: true }
        );
      }

      console.log('[AI] Content blocks:', JSON.stringify(content).slice(0, 500));
      ...
      if (toolUseBlocks.length === 0) {
        ...
        if (!textContent) {
          console.error('[AI] No text content found. Content structure:', JSON.stringify(content).slice(0, 500));
          return buildInsightFromAnalysis(
            { summary: 'Analysis complete - no detailed text response', confidence: 0.3 },
        ...
        try {
          ...
          return buildInsightFromAnalysis(analysis, {
      ...
      console.log(`🔧 Model requested ${toolUseBlocks.length} tool calls`);
      ...
      for (const toolUseBlock of toolUseBlocks) {
        ...
        toolCallCount++;
        const { id, name, input } = getToolDetails(toolUseBlock);
        const result = await executeTool(name, input);
        toolResults.push(formatToolResult(id, result));
      ...
      if (toolResults.length > 0) {
        messages[messages.length - 1].content.push(
          formatTextBlock('Respond with ONLY the JSON object. No preamble text. Start with { end with }.')
        );
      }

      if (needsFinalResponse) {
        console.log(`🛑 Tool call limit reached, requesting final analysis`);
        messages.push({
          role: 'user',
          content: [formatTextBlock('Maximum tool calls reached. Provide your final analysis now based on the data collected.')]
        });
      }
 ...
 * @deprecated Superseded by buildTargetedPromptWithData (single-shot, no tool loop)
 */
 ...
 * @deprecated Superseded by buildSummaryPromptWithData (single-shot, no tool loop)
 */
...
async function callBedrockSingleShot(prompt, maxTokens = 2000) {
  ...
  factoryState.tokenUsage.inputTokens += usage.input_tokens || 0;
  factoryState.tokenUsage.outputTokens += usage.output_tokens || 0;
  factoryState.tokenUsage.callCount += 1;
  console.log(`[AI-SingleShot] Tokens: in=${usage.input_tokens || 0}, out=${usage.output_tokens || 0}, daily_total_in=${factoryState.tokenUsage.inputTokens}`);
 ...
 * @deprecated Superseded by callBedrockSingleShot (single-shot, no tool loop)
 */
async function callClaudeWithPrompt(prompt, trends, maxToolCalls) {
  try {
    ...
    while (true) {
      ...
      const responseBody = JSON.parse(new TextDecoder().decode(response.body));
      console.log('[AI-Tier3] Raw Bedrock response:', JSON.stringify(responseBody).slice(0, 1000));
      const content = extractResponseContent(responseBody);

      if (!content || content.length === 0) {
        console.error('[AI-Tier3] No content array in response');
        return buildInsightFromAnalysis(
          { summary: 'Analysis temporarily unavailable - unexpected API response', confidence: 0.1 },
          { dataPoints: trends.length, apiError: true }
        );
      ...
      console.log('[AI-Tier3] Content blocks:', JSON.stringify(content).slice(0, 500));
      const toolUseBlocks = extractToolUseBlocks(content);

      if (toolUseBlocks.length === 0) {
        const responseText = extractTextContent(content);
        if (!responseText) {
          console.error('[AI-Tier3] No text content found. Content:', JSON.stringify(content).slice(0, 500));
          return buildInsightFromAnalysis(
            { summary: 'Analysis complete - no detailed text response', confidence: 0.3 },
            { dataPoints: trends.length, noTextBlock: true }
          );
        ...
        try {
          ...
          return buildInsightFromAnalysis(JSON.parse(jsonText), {
            dataPoints: trends.length,
            toolCallsUsed: toolCallCount
          });
        } catch (parseError) {
      ...
      if (needsFinalResponse) {
        messages.push({
          role: 'user',
          content: [formatTextBlock('Maximum tool calls reached. Provide your final analysis now.')]
        });
      }
    }
  } catch (error) {
...
async function runTrendAnalysis() {
  ...
  try {
    ...
    if (insight) {
      // Reuse shared insight processing (broadcast, store, CMMS, vector store)
      processInsight(insight, trends);
    }
