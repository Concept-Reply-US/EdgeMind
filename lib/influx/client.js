...
async function setRetentionPolicy(hours) {
  ...
  // shardGroupDurationSeconds must be <= retention; use 1 hour for short retention windows
  const retentionSeconds = hours * 3600;
  const shardGroupSeconds = Math.min(retentionSeconds, 3600);
  const retentionRules = hours > 0
    ? [{ type: 'expire', everySeconds: retentionSeconds, shardGroupDurationSeconds: shardGroupSeconds }]
    : [];
...
/**
 * Clears all data by dropping and recreating the bucket.
 * This is instant (no tombstone compaction) unlike the Delete API.
 * Preserves the current retention policy.
 * WARNING: This operation cannot be undone.
 *
 * @returns {Promise<void>}
 * @throws {Error} If drop/recreate fails
 */
async function clearBucket() {
  const http = require('http');
  const url = new URL(CONFIG.influxdb.url);

  // Get current bucket info to preserve retention settings
  const bucketInfo = await getBucketInfo();
  const bucketId = bucketInfo.id;
  ...
  await new Promise((resolve, reject) => {
    const req = http.request({
      ...
      path: `/api/v2/buckets/${bucketId}`,
      method: 'DELETE',
      headers: { 'Authorization': `Token ${CONFIG.influxdb.token}` }
    }, (res) => {
      let data = '';
      res.on('data', (chunk) => { data += chunk; });
      res.on('end', () => {
        if (res.statusCode !== 204 && res.statusCode !== 200) {
          return reject(new Error(`InfluxDB bucket delete error: ${res.statusCode} - ${data}`));
        }
        resolve();
      });
    });
  ...
  // Step 2: Recreate the bucket with the same name and retention
  const createBody = JSON.stringify({
    name: CONFIG.influxdb.bucket,
    ...
    const req = http.request({
      ...
      res.on('end', () => {
        if (res.statusCode !== 201 && res.statusCode !== 200) {
          return reject(new Error(`InfluxDB bucket create error: ${res.statusCode} - ${data}`));
        }
        resolve();
      });
    });
