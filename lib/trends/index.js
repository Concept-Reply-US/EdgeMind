...
async function queryMultiMeasurementTrend(measurements, window, enterprise, aggregateFn = 'sum') {
  ...
  const VALID_AGGREGATE_FNS = ['mean', 'sum', 'last', 'max', 'min', 'median'];
  const safeFn = VALID_AGGREGATE_FNS.includes(aggregateFn) ? aggregateFn : 'sum';
  ...
  const fluxQuery = `
    from(bucket: "${CONFIG.influxdb.bucket}")
      ...
      ${enterpriseFilter}
      |> filter(fn: (r) => r._value >= 0)
      |> aggregateWindow(every: ${windowConfig.aggregation}, fn: ${safeFn}, createEmpty: false)
      |> group(columns: ["_time"])
      |> ${safeFn}()
      |> group()
      |> sort(columns: ["_time"])
...
async function calculateDowntimePareto(enterprise = 'ALL', window = 'daily', limit = 10) {
  ...
  // Get state measurements from schema cache for the requested enterprise
  const { schemaCache } = require('../state');
  const stateMeasurements = [];

  // Check if schema cache is populated
  if (schemaCache.measurements && schemaCache.measurements.size > 0) {
    for (const [name, meta] of schemaCache.measurements.entries()) {
      // Check if this is a state/status measurement
      const isStateType = meta.classification === 'state_status' ||
                         name.toLowerCase().includes('state') ||
                         name.toLowerCase().includes('status');

      // Check if it matches the requested enterprise
      const matchesEnterprise = enterprise === 'ALL' ||
                               (meta.enterprises && meta.enterprises.includes(enterprise));

      if (isStateType && matchesEnterprise) {
        stateMeasurements.push(name);
      }
    }
  }

  // If no state measurements found in cache, fall back to suffix matching
  let measurementFilter;
  if (stateMeasurements.length === 0) {
    console.warn(`[TRENDS] No state measurements found in schema cache for ${enterprise}, using suffix fallback`);
    measurementFilter = `
      |> filter(fn: (r) => strings.hasSuffix(v: r._measurement, suffix: "_state") or
                          strings.hasSuffix(v: r._measurement, suffix: "_STATE") or
                          strings.hasSuffix(v: r._measurement, suffix: "_State") or
                          strings.hasSuffix(v: r._measurement, suffix: "_status") or
                          strings.hasSuffix(v: r._measurement, suffix: "_STATUS") or
                          strings.hasSuffix(v: r._measurement, suffix: "_Status"))`;
  } else {
    // Build exact measurement filter from schema cache
    const measurementMatches = stateMeasurements
      .map(m => `r._measurement == "${sanitizeInfluxIdentifier(m)}"`)
      .join(' or\n          ');
    measurementFilter = `
      |> filter(fn: (r) => ${measurementMatches})`;
  }
  ...
  const fluxQuery = `
    ...
    numericDowntime = from(bucket: "${CONFIG.influxdb.bucket}")
      ...
      |> filter(fn: (r) => r._value >= 0 and r._value <= 5)
      ${groupAndCount}
...
async function calculateWasteTrend(enterprise = 'ALL', window = 'shift') {
  ...
  for (const ent of enterprises) {
    ...
    const entContext = ENTERPRISE_DOMAIN_CONTEXT[ent];
    ...
    // Issue #10 fix: Pull thresholds from factoryState if available
    const { factoryState } = require('../state');
    const criticalThreshold = factoryState.thresholdSettings?.waste?.critical ?? 1000;
    const warningThreshold = factoryState.thresholdSettings?.waste?.warning ?? 750;
    let alert = null;
