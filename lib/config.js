 ...
 *
 * @property {Object} agentcore - AWS Bedrock AgentCore configuration
 */
const CONFIG = {
  mqtt: {
  ...
  bedrock: {
    ...
    tierModelId: process.env.BEDROCK_TIER_MODEL_ID || 'us.anthropic.claude-3-5-haiku-20241022-v1:0'
  },
  influxdb: {
    url: process.env.INFLUXDB_URL || 'http://localhost:8086',
    token: process.env.INFLUXDB_TOKEN || process.env.INFLUXDB_ADMIN_TOKEN || '',
  ...
  demo: {
    ...
    maxConcurrentInjections: 3,
    namespace: 'conceptreply'
  },
  cesmii: {
    enabled: process.env.CESMII_ENABLED !== 'false',
    profilesDir: path.join(__dirname, 'cesmii', 'profiles'),
    publishTopicPrefix: 'edgemind',
    maxWorkOrders: 100,
    validationStrict: false,
    ignoredProfileTypes: (process.env.CESMII_IGNORED_PROFILES || 'AssetOEEV1,FlowComputer').split(',').map(s => s.trim()).filter(Boolean)
  }
};
