...
async function queryProductionVsTarget(enterprise = 'ALL', window = 'shift') {
  ...
  // Build measurement filter — use domain-specific metrics if available
  let measurementFilter;
  const domainCtx = enterprise !== 'ALL' ? ENTERPRISE_DOMAIN_CONTEXT[enterprise] : null;
  const prodMetrics = domainCtx?.productionMetrics;

  if (prodMetrics && prodMetrics.length > 0) {
    const conditions = prodMetrics.map(m =>
      `r._measurement == "${sanitizeInfluxIdentifier(m)}"`
    ).join(' or ');
    measurementFilter = `|> filter(fn: (r) => ${conditions})`;
  } else if (enterprise === 'ALL') {
    measurementFilter = `|> filter(fn: (r) => strings.containsStr(v: r._measurement, substr: "count") or
                          strings.containsStr(v: r._measurement, substr: "COUNT") or
                          strings.containsStr(v: r._measurement, substr: "production"))`;
  } else {
    // No production metrics defined for this enterprise
    return {
      byLine: [],
      summary: { totalActual: 0, totalTarget: 0, overallPercent: 0, linesOnTarget: 0, linesBelowTarget: 0 },
...
async function queryEnergyConsumption(enterprise = 'ALL', window = 'shift') {
  ...
  // Check if enterprise has energy metrics defined
  const energyCtx = enterprise !== 'ALL' ? ENTERPRISE_DOMAIN_CONTEXT[enterprise] : null;
  const energyMetrics = energyCtx?.energyMetrics;

  // If enterprise-specific and no energy metrics defined, return empty immediately
  if (enterprise !== 'ALL' && energyMetrics && energyMetrics.length === 0) {
    return {
      byLine: [],
      summary: { totalConsumption: 0, averageConsumption: 0, risingTrend: 0, stableLines: 0, fallingLines: 0, unit: 'kW' },
      window: validateTimeWindow(window),
      timestamp: new Date().toISOString()
    };
  }
  ...
  let energyMeasurementFilter;
  if (energyMetrics && energyMetrics.length > 0) {
    const conditions = energyMetrics.map(m =>
      `r._measurement == "${sanitizeInfluxIdentifier(m)}"`
    ).join(' or ');
    energyMeasurementFilter = `|> filter(fn: (r) => ${conditions})`;
  } else {
    // Broaden search to include electrical measurements as power proxies
    energyMeasurementFilter = `|> filter(fn: (r) =>
                          strings.containsStr(v: r._measurement, substr: "power") or
                          strings.containsStr(v: r._measurement, substr: "Power") or
                          strings.containsStr(v: r._measurement, substr: "energy") or
                          strings.containsStr(v: r._measurement, substr: "Energy") or
                          strings.containsStr(v: r._measurement, substr: "kW") or
                          strings.containsStr(v: r._measurement, substr: "KW") or
                          strings.containsStr(v: r._measurement, substr: "watts") or
                          strings.containsStr(v: r._measurement, substr: "Watts") or
                          strings.containsStr(v: r._measurement, substr: "voltage") or
                          strings.containsStr(v: r._measurement, substr: "Voltage") or
                          strings.containsStr(v: r._measurement, substr: "current") or
                          strings.containsStr(v: r._measurement, substr: "Current") or
                          strings.containsStr(v: r._measurement, substr: "ampere") or
                          strings.containsStr(v: r._measurement, substr: "Ampere"))`;
  }
