...
// Runtime ARNs - loaded from SSM on demand
let RUNTIME_ARNS = { chat: null, troubleshoot: null, anomaly: null };
...
// Load agent ARNs from SSM Parameter Store
async function loadAgentArnsFromSSM(force = false) {
  ...
  try {
    ...
    for (const param of response.Parameters || []) {
      ...
      if (Object.hasOwn(RUNTIME_ARNS, agentType)) {
        RUNTIME_ARNS[agentType] = param.Value;
      }
    }
    ssmLastLoaded = Date.now();
    console.log('[AgentCore] Loaded agent ARNs from SSM:', RUNTIME_ARNS);
  } catch (err) {
    console.warn('[AgentCore] Could not load agent ARNs from SSM:', err.message);
  }
}
...
async function invoke(agentType, prompt, sessionId, stream = false, messages = []) {
  if (useRuntime && client) {
    // Load/refresh SSM params
    await loadAgentArnsFromSSM();
    
    const { InvokeAgentRuntimeCommand } = require('@aws-sdk/client-bedrock-agentcore');
    const runtimeArn = RUNTIME_ARNS[agentType];
    ...
    const command = new InvokeAgentRuntimeCommand({
      ...
      payload: Buffer.from(JSON.stringify({ prompt, messages }), 'utf-8'),
    ...
    const response = await fetch(`${url}/invocations`, {
      ...
      body: JSON.stringify({ prompt, session_id: sessionId || 'default', messages })
    });
    return { response, isRuntime: false };
  }
}

module.exports = { invoke, useRuntime, RUNTIME_ARNS, LOCAL_URLS, getRuntimeArns: () => RUNTIME_ARNS };
