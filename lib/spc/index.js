...
async function discoverSPCMeasurements(enterprise, limit = 5, site = null) {
  ...
  for (const measurement of candidates.slice(0, 15)) { // Analyze top 15
    try {
...
async function querySPCData(measurement, window, enterprise, site = null) {
  ...
  // Auto-calculate spec limits at ±4σ (wider than control limits for realistic Cpk)
  // This ensures a process within ±3σ (in control) has Cpk ≈ 1.33
  const target = stats.mean;
  const tolerance = 4 * stats.stdDev;
  const lsl = target - tolerance;
  const usl = target + tolerance;
