// lib/oee/index.js - OEE (Overall Equipment Effectiveness) Module

const { queryApi } = require('../influx/client');
const { schemaCache } = require('../state');
const { refreshSchemaCache } = require('../schema');
const CONFIG = require('../config');
const { sanitizeInfluxIdentifier } = require('../validation');
const { getFluxSiteFilter } = require('../factory-sites');

/** Decimal-to-percent threshold: values at or below this are treated as decimal (0-1 range) */
const DECIMAL_PERCENT_THRESHOLD = 1.5;

// ---------------------------------------------------------------------------
// P4: Result-level cache for calculateOEEv2
// Prevents redundant heavy InfluxDB queries when the same enterprise OEE is
// requested multiple times within a short burst (e.g., breakdown + status
// endpoints both calling calculateOEEv2 for the same enterprise).
// ---------------------------------------------------------------------------
const OEE_CALC_CACHE_TTL_MS = 30_000; // 30 seconds
const oeeCalcCache = new Map(); // "<enterprise>:<site>" -> { data, expiresAt }

// =============================================================================
// PHASE 1 & 2: ROBUST OEE CONFIGURATION & DISCOVERY
// =============================================================================

/**
 * OEE Calculation Tiers
 * Defines the priority hierarchy for calculating OEE metrics
 */
const OEE_TIERS = {
  1: 'pre-computed-overall',
  2: 'pre-computed-components',
  3: 'calculated-from-raw',
  4: 'insufficient-data'
};

/**
 * OEE measurement patterns for auto-discovery
 * Regular expressions to match OEE-related measurements
 */
const OEE_PATTERNS = {
  overall: [/^oee$/i, /metric_oee/i, /oee_overall/i],
  availability: [/oee_availability/i, /availability/i],
  performance: [/oee_performance/i, /performance/i],
  quality: [/oee_quality/i, /quality/i]
};

/**
 * Runtime OEE configuration (populated by discovery)
 * Contains per-enterprise OEE calculation strategies
 */
const oeeConfig = {
  defaults: {
    staleDataThreshold: 300000, // 5 minutes
    decimalToPercentThreshold: 1.5
  },
  enterprises: {} // Populated by discoverOEESchema()
};

/**
 * Discovery state tracking for resilience
 */
const discoveryState = {
  inProgress: null, // Promise for in-flight discovery (concurrency guard)
  lastAttempt: 0, // Timestamp of last discovery attempt
  lastSuccess: 0, // Timestamp of last successful discovery
  failureCount: 0, // Consecutive failure counter
  cooldownMs: 30000, // 30 seconds cooldown after failure
  maxFailures: 3 // After this many failures, use fallback config
};

/**
 * Fallback OEE measurement names (from queryFactoryStatus hardcoded patterns)
 * Used when schema discovery fails repeatedly
 */
const FALLBACK_MEASUREMENTS = {
  'Enterprise A': {
    overall: null,
    availability: 'OEE_Availability',
    performance: 'OEE_Performance',
    quality: 'OEE_Quality'
  },
  'Enterprise B': {
    overall: 'metric_oee',
    availability: 'metric_availability',
    performance: 'metric_performance',
    quality: 'metric_quality'
  }
};

/**
 * Discovers OEE schema for all enterprises by analyzing available measurements.
 * Populates the oeeConfig.enterprises object with tier-based calculation strategies.
 * Includes concurrency guard, retry backoff, and fallback to hardcoded measurements.
 *
 * @returns {Promise<Object>} Discovered OEE configuration per enterprise
 */
async function discoverOEESchema() {
  // Concurrency guard: if discovery is already in progress, await that promise
  if (discoveryState.inProgress) {
    console.debug('[OEE] Discovery already in progress, awaiting existing operation');
    return discoveryState.inProgress;
  }

  // Check cooldown: don't retry if we're in cooldown period after a failure
  const now = Date.now();
  const timeSinceLastAttempt = now - discoveryState.lastAttempt;
  if (discoveryState.failureCount > 0 && timeSinceLastAttempt < discoveryState.cooldownMs) {
    const remainingCooldown = Math.ceil((discoveryState.cooldownMs - timeSinceLastAttempt) / 1000);
    console.debug(`[OEE] In cooldown period, ${remainingCooldown}s remaining. Using existing config.`);

    // If we have a fallback config and hit max failures, use it
    if (discoveryState.failureCount >= discoveryState.maxFailures && Object.keys(oeeConfig.enterprises).length === 0) {
      console.debug('[OEE] Max failures reached, using fallback config');
      applyFallbackConfig();
    }

    return oeeConfig.enterprises;
  }

  // Start discovery (store promise for concurrency guard)
  discoveryState.inProgress = _doDiscoverOEESchema();

  try {
    const result = await discoveryState.inProgress;
    return result;
  } finally {
    discoveryState.inProgress = null;
  }
}

/**
 * Internal discovery implementation (wrapped by concurrency guard)
 * @private
 */
async function _doDiscoverOEESchema() {
  discoveryState.lastAttempt = Date.now();

  try {
    await refreshSchemaCache();
  } catch (refreshError) {
    console.error('[OEE] Schema refresh failed during discovery:', refreshError.message);
    discoveryState.failureCount++;

    // If we have existing config, keep it and return early
    if (Object.keys(oeeConfig.enterprises).length > 0) {
      console.debug('[OEE] Keeping existing config after schema refresh failure');
      return oeeConfig.enterprises;
    }

    // If we hit max failures, apply fallback config
    if (discoveryState.failureCount >= discoveryState.maxFailures) {
      console.error(`[OEE] Schema refresh failed ${discoveryState.failureCount} times, using fallback config`);
      applyFallbackConfig();
      return oeeConfig.enterprises;
    }

    // No existing config and not yet at max failures - throw to signal discovery failed
    throw new Error(`Schema refresh failed (attempt ${discoveryState.failureCount}/${discoveryState.maxFailures}): ${refreshError.message}`);
  }

  // Schema refresh succeeded, proceed with discovery
  const allMeasurements = Array.from(schemaCache.measurements.values());

  // If schemaCache is empty (InfluxDB has no data), use fallback
  if (allMeasurements.length === 0) {
    console.debug('[OEE] Schema cache is empty, using fallback config');
    applyFallbackConfig();
    return oeeConfig.enterprises;
  }

  // Group by enterprise
  const byEnterprise = {};
  allMeasurements.forEach(m => {
    m.enterprises.forEach(ent => {
      if (!byEnterprise[ent]) byEnterprise[ent] = [];
      byEnterprise[ent].push(m);
    });
  });

  // Analyze each enterprise
  const discovered = {};
  for (const [enterprise, measurements] of Object.entries(byEnterprise)) {
    discovered[enterprise] = analyzeEnterpriseOEE(enterprise, measurements);
  }

  // Only update config if we discovered at least one enterprise
  if (Object.keys(discovered).length > 0) {
    oeeConfig.enterprises = discovered;
    discoveryState.failureCount = 0; // Reset failure counter on success
    discoveryState.lastSuccess = Date.now();
    console.debug('[OEE] Discovery complete:', Object.keys(discovered).map(e =>
      `${e}: Tier ${discovered[e].tier}`).join(', '));
  } else {
    console.debug('[OEE] No enterprises discovered, using fallback config');
    applyFallbackConfig();
  }

  return oeeConfig.enterprises;
}

/**
 * Applies fallback OEE configuration using hardcoded measurement names
 * @private
 */
function applyFallbackConfig() {
  const fallbackConfig = {};

  for (const [enterprise, measurements] of Object.entries(FALLBACK_MEASUREMENTS)) {
    const tier = measurements.overall ? 1 : 2;
    fallbackConfig[enterprise] = {
      tier,
      measurements,
      valueFormat: 'unknown', // Will be detected at runtime
      sites: [], // Unknown in fallback mode
      lastDiscovery: new Date().toISOString(),
      confidence: 0.70, // Lower confidence for fallback
      reason: `Using fallback measurement names (schema discovery unavailable)`,
      fallback: true
    };
  }

  oeeConfig.enterprises = fallbackConfig;
  console.debug('[OEE] Fallback config applied for enterprises:', Object.keys(fallbackConfig).join(', '));
}

/**
 * Real production sites (not vendor sites like "aveva", "hivemq", "broker")
 */
const REAL_SITES = ['Dallas', 'Site1', 'Site2', 'Site3'];

/**
 * Analyzes a single enterprise's measurements to determine OEE calculation tier.
 * Filters measurements to only those present in real production sites.
 *
 * @param {string} enterprise - Enterprise name
 * @param {Array} measurements - Array of measurement metadata objects
 * @returns {Object} OEE configuration for this enterprise
 */
function analyzeEnterpriseOEE(enterprise, measurements) {
  // Filter to measurements that have data from real sites
  const realSiteMeasurements = measurements.filter(m =>
    m.sites && m.sites.some(site => REAL_SITES.includes(site))
  );

  console.debug(`[OEE Discovery] ${enterprise}: Filtered ${measurements.length} measurements to ${realSiteMeasurements.length} with real site data`);

  const found = { overall: null, availability: null, performance: null, quality: null };

  for (const measurement of realSiteMeasurements) {
    const name = measurement.name;
    if (OEE_PATTERNS.overall.some(p => p.test(name))) found.overall = name;
    if (OEE_PATTERNS.availability.some(p => p.test(name))) found.availability = name;
    if (OEE_PATTERNS.performance.some(p => p.test(name))) found.performance = name;
    if (OEE_PATTERNS.quality.some(p => p.test(name))) found.quality = name;
  }

  let tier, confidence, reason;
  if (found.overall) {
    tier = 1; confidence = 0.95;
    reason = `Using pre-computed OEE from ${found.overall}`;
  } else if (found.availability && found.performance && found.quality) {
    tier = 2; confidence = 0.90;
    reason = 'Calculating from A x P x Q components';
  } else {
    tier = 4; confidence = 0.0;
    reason = 'Insufficient OEE data available';
  }

  // Infer value format (NOTE: This is a hint only, actual normalization uses runtime detection)
  let valueFormat = 'unknown';
  const oeeRelated = realSiteMeasurements.filter(m =>
    m.name === found.overall || m.name === found.availability ||
    m.name === found.performance || m.name === found.quality
  );
  for (const m of oeeRelated) {
    if (m.sampleValues?.length > 0) {
      console.debug(`[OEE Discovery] ${enterprise} - ${m.name} sampleValues:`, m.sampleValues);
      if (m.sampleValues.every(v => typeof v === 'number' && v <= DECIMAL_PERCENT_THRESHOLD)) {
        valueFormat = 'decimal'; break;
      }
      if (m.sampleValues.some(v => typeof v === 'number' && v > DECIMAL_PERCENT_THRESHOLD)) {
        valueFormat = 'percentage'; break;
      }
    }
  }

  console.debug(`[OEE Discovery] ${enterprise}: Tier ${tier}, valueFormat="${valueFormat}", measurements=`, found);

  return {
    tier,
    measurements: found,
    valueFormat,
    sites: [...new Set(realSiteMeasurements.flatMap(m => m.sites || []))],
    lastDiscovery: new Date().toISOString(),
    confidence,
    reason
  };
}

// =============================================================================
// PHASE 3: TIER 1 & 2 OEE CALCULATORS
// =============================================================================

/**
 * Normalize an OEE value to a 0-100 percentage.
 * Values <= 1.5 are assumed decimal (0.72 => 72%), values > 1.5 are already percentages.
 * Shared by Tier 1 and Tier 2 calculators.
 *
 * @param {number|null} val - Raw OEE component value
 * @param {string} componentName - Label for debug logging
 * @param {string} [requestId=''] - Request ID for log correlation
 * @returns {number|null} Normalized value clamped to 0-100, or null if input is null
 */
function normalizeOEEValue(val, componentName, requestId = '') {
  if (val === null) return null;
  const isDecimal = val <= DECIMAL_PERCENT_THRESHOLD;
  const normalized = isDecimal ? val * 100 : val;
  console.debug(`[OEE:${requestId}] ${componentName}: raw=${val}, isDecimal=${isDecimal}, normalized=${normalized}`);
  return Math.min(100, Math.max(0, normalized));
}

/**
 * Main OEE calculation function using tiered strategy
 * @param {string} enterprise - Enterprise name
 * @param {string|null} site - Optional site filter
 * @returns {Promise<Object>} OEE result with calculation metadata
 */
async function calculateOEEv2(enterprise, site = null) {
  const requestId = Math.random().toString(36).substring(2, 8);
  console.debug(`[OEE:${requestId}] calculateOEEv2 called for enterprise="${enterprise}", site="${site}"`);

  // P4: Return cached result if fresh (avoids duplicate InfluxDB queries within 30s)
  const cacheKey = `${enterprise}:${site ?? ''}`;
  const cached = oeeCalcCache.get(cacheKey);
  if (cached && Date.now() <= cached.expiresAt) {
    console.debug(`[OEE:${requestId}] Cache hit for ${cacheKey}`);
    return cached.data;
  }

  // Run discovery if not done yet
  if (Object.keys(oeeConfig.enterprises).length === 0) {
    console.debug(`[OEE:${requestId}] No cached config, running discovery...`);
    try {
      await discoverOEESchema();
    } catch (discoveryError) {
      console.error(`[OEE:${requestId}] Discovery failed, returning insufficient data result:`, discoveryError.message);
      return createOEEResult(enterprise, site, null, null, 4, 'OEE schema discovery failed (InfluxDB unavailable)');
    }
  }

  const config = oeeConfig.enterprises[enterprise];
  if (!config) {
    console.debug(`[OEE:${requestId}] Enterprise not found in schema`);
    return createOEEResult(enterprise, site, null, null, 4, 'Enterprise not found in schema');
  }

  console.debug(`[OEE:${requestId}] Using Tier ${config.tier}, valueFormat="${config.valueFormat}", measurements:`, config.measurements);

  let result;
  switch (config.tier) {
    case 1: result = await calculateTier1(enterprise, site, config, requestId); break;
    case 2: result = await calculateTier2(enterprise, site, config, requestId); break;
    default: result = createOEEResult(enterprise, site, null, null, 4, config.reason);
  }

  // P4: Store result in cache before returning
  oeeCalcCache.set(cacheKey, { data: result, expiresAt: Date.now() + OEE_CALC_CACHE_TTL_MS });
  return result;
}

/**
 * Tier 1: Use pre-computed overall OEE
 * Also queries A/P/Q components if available for diagnostic purposes
 */
async function calculateTier1(enterprise, site, config, requestId = 'unknown') {
  const measurement = config.measurements.overall;

  const fluxQuery = `
    import "strings"

    from(bucket: "${CONFIG.influxdb.bucket}")
      |> range(start: -24h)
      |> filter(fn: (r) => r._field == "value")
      |> filter(fn: (r) => r._measurement == "${sanitizeInfluxIdentifier(measurement)}")
      |> filter(fn: (r) => r.enterprise == "${enterprise}")
      ${site ? `|> filter(fn: (r) => r.site == "${site}")` : ''}
      ${getFluxSiteFilter()}
      |> filter(fn: (r) => r._value > 0)
      |> group()
      |> mean()
  `;

  let oeeValue = null;
  let rawOeeValue = null;
  let dataPoints = 0;

  await new Promise((resolve) => {
    queryApi.queryRows(fluxQuery, {
      next(row, tableMeta) {
        const o = tableMeta.toObject(row);
        if (o._value !== undefined) {
          oeeValue = o._value;
          rawOeeValue = o._value;
          dataPoints++;
        }
      },
      error(error) {
        console.error(`[OEE:${requestId}] Tier 1 OEE query error for ${enterprise}:`, error.message);
        resolve();
      },
      complete() { resolve(); }
    });
  });

  console.debug(`[OEE:${requestId}] Tier 1 raw OEE value: ${rawOeeValue}, cached valueFormat: ${config.valueFormat}`);
  oeeValue = normalizeOEEValue(oeeValue, 'overall-oee', requestId);

  // Query A/P/Q components if available (for diagnostic purposes)
  let components = null;
  const measurementsUsed = [measurement];
  const { availability: availMeasure, performance: perfMeasure, quality: qualMeasure } = config.measurements;

  if (availMeasure && perfMeasure && qualMeasure) {
    const queryComponent = async (componentMeasurement, componentName) => {
      const componentQuery = `
        import "strings"

        from(bucket: "${CONFIG.influxdb.bucket}")
          |> range(start: -24h)
          |> filter(fn: (r) => r._field == "value")
          |> filter(fn: (r) => r._measurement == "${sanitizeInfluxIdentifier(componentMeasurement)}")
          |> filter(fn: (r) => r.enterprise == "${enterprise}")
          ${site ? `|> filter(fn: (r) => r.site == "${site}")` : ''}
          ${getFluxSiteFilter()}
          |> filter(fn: (r) => r._value > 0)
          |> group()
          |> mean()
      `;

      return new Promise((resolve) => {
        let value = null;
        queryApi.queryRows(componentQuery, {
          next(row, tableMeta) {
            const o = tableMeta.toObject(row);
            if (o._value !== undefined) value = o._value;
          },
          error(error) {
            console.error(`[OEE:${requestId}] Tier 1 component query error for ${componentMeasurement}:`, error.message);
            resolve(null);
          },
          complete() {
            console.debug(`[OEE:${requestId}] Tier 1 ${componentName} raw value: ${value}`);
            resolve(value);
          }
        });
      });
    };

    // Query all components in parallel
    const [availability, performance, quality] = await Promise.all([
      queryComponent(availMeasure, 'availability'),
      queryComponent(perfMeasure, 'performance'),
      queryComponent(qualMeasure, 'quality')
    ]);

    const normAvail = normalizeOEEValue(availability, 'availability', requestId);
    const normPerf = normalizeOEEValue(performance, 'performance', requestId);
    const normQual = normalizeOEEValue(quality, 'quality', requestId);

    // Only include components if at least one was found
    if (normAvail !== null || normPerf !== null || normQual !== null) {
      components = {
        availability: normAvail !== null ? parseFloat(normAvail.toFixed(1)) : null,
        performance: normPerf !== null ? parseFloat(normPerf.toFixed(1)) : null,
        quality: normQual !== null ? parseFloat(normQual.toFixed(1)) : null
      };
      measurementsUsed.push(availMeasure, perfMeasure, qualMeasure);
      dataPoints += 3;
    }
  }

  console.debug(`[OEE:${requestId}] Tier 1 final result: OEE=${oeeValue}, components=`, components);

  return createOEEResult(enterprise, site, oeeValue, components, 1, config.reason, {
    measurementsUsed,
    dataPoints
  });
}

/**
 * Tier 2: Calculate from A x P x Q components
 */
async function calculateTier2(enterprise, site, config, requestId = 'unknown') {
  const { availability: availMeasure, performance: perfMeasure, quality: qualMeasure } = config.measurements;

  const queryComponent = async (measurement, componentName) => {
    const fluxQuery = `
      import "strings"

      from(bucket: "${CONFIG.influxdb.bucket}")
        |> range(start: -24h)
        |> filter(fn: (r) => r._field == "value")
        |> filter(fn: (r) => r._measurement == "${sanitizeInfluxIdentifier(measurement)}")
        |> filter(fn: (r) => r.enterprise == "${enterprise}")
        ${site ? `|> filter(fn: (r) => r.site == "${site}")` : ''}
        ${getFluxSiteFilter()}
        |> filter(fn: (r) => r._value > 0)
        |> group()
        |> mean()
    `;

    return new Promise((resolve) => {
      let value = null;
      queryApi.queryRows(fluxQuery, {
        next(row, tableMeta) {
          const o = tableMeta.toObject(row);
          if (o._value !== undefined) value = o._value;
        },
        error(error) {
          console.error(`[OEE:${requestId}] Component query error for ${measurement}:`, error.message);
          resolve(null);
        },
        complete() {
          console.debug(`[OEE:${requestId}] Tier 2 ${componentName} raw value: ${value}`);
          resolve(value);
        }
      });
    });
  };

  // Query all components in parallel
  const [availability, performance, quality] = await Promise.all([
    queryComponent(availMeasure, 'availability'),
    queryComponent(perfMeasure, 'performance'),
    queryComponent(qualMeasure, 'quality')
  ]);

  console.debug(`[OEE:${requestId}] Tier 2 raw values: A=${availability}, P=${performance}, Q=${quality}`);

  const normAvail = normalizeOEEValue(availability, 'availability', requestId);
  const normPerf = normalizeOEEValue(performance, 'performance', requestId);
  const normQual = normalizeOEEValue(quality, 'quality', requestId);

  // Calculate OEE = A x P x Q
  let oeeValue = null;
  if (normAvail !== null && normPerf !== null && normQual !== null) {
    oeeValue = (normAvail / 100) * (normPerf / 100) * (normQual / 100) * 100;
    console.debug(`[OEE:${requestId}] Tier 2 calculated OEE: (${normAvail}/100) * (${normPerf}/100) * (${normQual}/100) * 100 = ${oeeValue}`);
  }

  console.debug(`[OEE:${requestId}] Tier 2 final result: OEE=${oeeValue}`);

  return createOEEResult(enterprise, site, oeeValue, {
    availability: normAvail !== null ? parseFloat(normAvail.toFixed(1)) : null,
    performance: normPerf !== null ? parseFloat(normPerf.toFixed(1)) : null,
    quality: normQual !== null ? parseFloat(normQual.toFixed(1)) : null
  }, 2, config.reason, {
    measurementsUsed: [availMeasure, perfMeasure, qualMeasure],
    dataPoints: 3
  });
}

/**
 * Creates a standardized OEE result object
 */
function createOEEResult(enterprise, site, oee, components, tier, reason, meta = {}) {
  return {
    enterprise,
    site,
    oee: oee !== null ? parseFloat(oee.toFixed(1)) : null,
    components,
    calculation: {
      tier,
      tierName: OEE_TIERS[tier],
      method: reason,
      measurementsUsed: meta.measurementsUsed || [],
      dataPoints: meta.dataPoints || 0,
      timeRange: { start: '-24h', end: 'now()' }
    },
    quality: {
      confidence: oee !== null ? (tier === 1 ? 0.95 : tier === 2 ? 0.90 : 0.0) : 0.0,
      status: oee !== null ? 'good' : 'unavailable'
    },
    timestamp: new Date().toISOString()
  };
}

// =============================================================================
// OEE QUERY FUNCTIONS (LEGACY API SUPPORT)
// =============================================================================

/**
 * Query 24h OEE aggregate for a specific enterprise or all enterprises
 * Delegates to calculateOEEv2 for correct tier-based calculation
 * @param {string} enterprise - Enterprise name or 'ALL'
 * @returns {Promise<Object>} OEE data
 */
async function queryOEE(enterprise) {
  // For 'ALL' enterprises, calculate the average of per-enterprise OEEs
  if (enterprise === 'ALL') {
    const breakdown = await queryOEEBreakdown();
    const enterprises = Object.values(breakdown.data);

    // Filter out enterprises with null OEE
    const validEnterprises = enterprises.filter(e => e.oee !== null && e.oee !== undefined);

    if (validEnterprises.length === 0) {
      return { average: null, period: '24h', enterprise: 'ALL', dataPoints: 0 };
    }

    const sum = validEnterprises.reduce((acc, e) => acc + e.oee, 0);
    const avg = sum / validEnterprises.length;

    return {
      average: parseFloat(avg.toFixed(1)),
      period: '24h',
      enterprise: 'ALL',
      dataPoints: validEnterprises.length
    };
  }

  // For specific enterprise, delegate to calculateOEEv2
  const result = await calculateOEEv2(enterprise);

  return {
    average: result.oee,
    period: '24h',
    enterprise,
    dataPoints: result.calculation?.dataPoints || 0
  };
}

/**
 * Query OEE breakdown by enterprise for the last 24 hours
 * Delegates to calculateOEEv2 for each enterprise
 * @returns {Promise<Object>} OEE data grouped by enterprise
 */
async function queryOEEBreakdown() {
  // Known enterprises (exclude Enterprise C as it uses ISA-88 batch control)
  const enterprises = ['Enterprise A', 'Enterprise B'];
  const data = {};

  // Query each enterprise using calculateOEEv2
  await Promise.all(
    enterprises.map(async (enterprise) => {
      try {
        const result = await calculateOEEv2(enterprise);
        if (result && result.oee !== null) {
          data[enterprise] = {
            oee: result.oee,
            dataPoints: result.calculation?.dataPoints || 0
          };
        }
      } catch (error) {
        console.error(`OEE breakdown error for ${enterprise}: ${error.message}`);
      }
    })
  );

  return {
    period: '24h',
    data
  };
}

/**
 * Query factory status with hierarchical enterprise/site OEE
 * @param {string} enterprise - Enterprise filter or 'ALL' (default: 'ALL')
 * @returns {Promise<Object>} Hierarchical OEE data
 */
async function queryFactoryStatus(enterprise = 'ALL') {
  // Build enterprise filter for Flux query
  const enterpriseFilter = (enterprise && enterprise !== 'ALL')
    ? `|> filter(fn: (r) => r.enterprise == "${sanitizeInfluxIdentifier(enterprise)}")`
    : '';

  // Use pivot-based query to get OEE + A/P/Q components separately (not blended)
  // This mirrors the correct approach from /api/oee/lines endpoint
  const fluxQuery = `
    import "strings"

    from(bucket: "${CONFIG.influxdb.bucket}")
      |> range(start: -24h)
      |> filter(fn: (r) => r._field == "value")
      |> filter(fn: (r) =>
        r._measurement == "OEE_Performance" or
        r._measurement == "OEE_Availability" or
        r._measurement == "OEE_Quality" or
        r._measurement == "metric_oee" or
        r._measurement == "metric_availability" or
        r._measurement == "metric_performance" or
        r._measurement == "metric_quality"
      )
      |> filter(fn: (r) => r._value > 0 and r._value <= 150)
      |> filter(fn: (r) => r.site == "Dallas" or r.site == "Dallas Line 1" or r.site == "Site1" or r.site == "Site2" or r.site == "Site3")
      ${enterpriseFilter}
      ${getFluxSiteFilter()}
      |> filter(fn: (r) => not strings.hasPrefix(v: r.full_topic, prefix: "AVEVA "))
      |> filter(fn: (r) => r.enterprise != "Enterprise C")
      |> filter(fn: (r) => r.site != "hivemq" and r.site != "broker" and r.site != "unknown")
      |> group(columns: ["enterprise", "site", "_measurement"])
      |> mean()
      |> group(columns: ["enterprise", "site"])
      |> pivot(rowKey: ["enterprise", "site"], columnKey: ["_measurement"], valueColumn: "_value")
  `;

  return new Promise((resolve) => {
    const sitesData = [];

    queryApi.queryRows(fluxQuery, {
      next(row, tableMeta) {
        const o = tableMeta.toObject(row);
        if (o.enterprise && o.site) {
          // Normalize value to percentage (handle both decimal 0-1 and percentage 0-100)
          const normalize = (val) => {
            if (val === undefined || val === null) return null;
            if (val > 0 && val <= DECIMAL_PERCENT_THRESHOLD) val = val * 100;
            return parseFloat(Math.min(100, Math.max(0, val)).toFixed(1));
          };

          // Extract values from either naming convention
          const availability = normalize(o.OEE_Availability ?? o.metric_availability);
          const performance = normalize(o.OEE_Performance ?? o.metric_performance);
          const quality = normalize(o.OEE_Quality ?? o.metric_quality);
          let oee = normalize(o.metric_oee);

          // Calculate OEE from components if not directly available
          if (oee === null && availability !== null && performance !== null && quality !== null) {
            oee = parseFloat(((availability / 100) * (performance / 100) * (quality / 100) * 100).toFixed(1));
          }

          // Skip sites with no OEE data
          if (oee === null && availability === null && performance === null && quality === null) {
            return;
          }

          sitesData.push({
            enterprise: o.enterprise,
            site: o.site,
            oee,
            availability,
            performance,
            quality
          });
        }
      },
      error(error) {
        console.error('InfluxDB factory status query error:', error.message);
        resolve({ enterprises: [], error: error.message });
      },
      complete() {
        // Group sites by enterprise and calculate enterprise-level OEE
        const enterpriseMap = {};

        sitesData.forEach(site => {
          if (!enterpriseMap[site.enterprise]) {
            enterpriseMap[site.enterprise] = {
              name: site.enterprise,
              sites: [],
              totalOee: 0,
              count: 0
            };
          }

          const enterprise = enterpriseMap[site.enterprise];
          const siteObj = {
            name: site.site,
            oee: site.oee,
            status: site.oee !== null ? (site.oee >= 80 ? 'healthy' : site.oee >= 60 ? 'warning' : 'critical') : 'unknown'
          };

          // Include A/P/Q if available
          if (site.availability !== null || site.performance !== null || site.quality !== null) {
            siteObj.availability = site.availability;
            siteObj.performance = site.performance;
            siteObj.quality = site.quality;
          }

          enterprise.sites.push(siteObj);

          // Only include sites with non-null OEE in enterprise average
          if (site.oee !== null) {
            enterprise.totalOee += site.oee;
            enterprise.count++;
          }
        });

        // Calculate average OEE per enterprise and determine status
        const enterprises = Object.values(enterpriseMap).map(ent => {
          const avgOee = ent.count > 0 ? ent.totalOee / ent.count : null;
          return {
            name: ent.name,
            oee: avgOee !== null ? parseFloat(avgOee.toFixed(1)) : null,
            status: avgOee !== null ? (avgOee >= 80 ? 'healthy' : avgOee >= 60 ? 'warning' : 'critical') : 'unknown',
            sites: ent.sites
          };
        });

        resolve({ enterprises });
      }
    });
  });
}

module.exports = {
  OEE_TIERS,
  OEE_PATTERNS,
  oeeConfig,
  discoverOEESchema,
  analyzeEnterpriseOEE,
  calculateOEEv2,
  createOEEResult,
  queryOEE,
  queryOEEBreakdown,
  queryFactoryStatus
};
