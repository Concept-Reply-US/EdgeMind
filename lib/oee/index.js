...
async function calculateTier1(enterprise, site, config, requestId = 'unknown') {
  ...
  if (oeeValue !== null && actualFormat === 'decimal') {
    ...
    console.debug(`[OEE:${requestId}] Detected decimal format (raw=${rawOeeValue}), normalized to ${oeeValue}%`);
  } else if (oeeValue !== null) {
    console.debug(`[OEE:${requestId}] Detected percentage format (raw=${rawOeeValue}), no normalization needed`);
  }
  if (oeeValue !== null) {
