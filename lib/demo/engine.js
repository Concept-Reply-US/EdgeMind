...
class ScenarioRunner {
  constructor() {
    ...
    this.onScenarioStart = null; // Callback for scenario start events
    this.onScenarioStop = null; // Callback for scenario stop events
  }

  /**
   * Initialize the runner with MQTT client and optional callbacks
   * @param {Object} params - Initialization parameters
   * @param {Object} params.mqttClient - Authenticated MQTT client
   * @param {Function} [params.onScenarioStart] - Callback invoked when scenario starts
   * @param {Function} [params.onScenarioStop] - Callback invoked when scenario stops
   */
  init({ mqttClient, onScenarioStart, onScenarioStop }) {
    this.mqttClient = mqttClient;
    this.onScenarioStart = onScenarioStart || null;
    this.onScenarioStop = onScenarioStop || null;
  ...
  start(scenarioId) {
    ...
    // Call onScenarioStart callback if provided (after scenario has started)
    if (this.onScenarioStart && typeof this.onScenarioStart === 'function') {
      try {
        this.onScenarioStart(scenario);
      } catch (callbackError) {
        console.error('[DEMO ENGINE] onScenarioStart callback error:', callbackError);
      }
    }
  ...
  stop() {
    ...
    // Issue 6 fix: Call onScenarioStop callback before clearing state
    if (this.onScenarioStop && typeof this.onScenarioStop === 'function') {
      try {
        this.onScenarioStop();
      } catch (callbackError) {
        console.error('[DEMO ENGINE] onScenarioStop callback error:', callbackError);
      }
    }
...
/**
 * Initialize both runners with MQTT client and optional callbacks
 * @param {Object} params - Initialization parameters
 * @param {Object} params.mqttClient - Authenticated MQTT client
 * @param {Function} [params.onScenarioStart] - Callback invoked when scenario starts
 * @param {Function} [params.onScenarioStop] - Callback invoked when scenario stops
 */
function init({ mqttClient, onScenarioStart, onScenarioStop }) {
  scenarioRunner.init({ mqttClient, onScenarioStart, onScenarioStop });
